...
The seven entity collections are periodically copied into the application database as a fallback cache. `ssdatas` and `sanghdatas` use `ENTITY_MONGO_URI` by default because person data may change. `entities`, `parententities`, `stharas`, `userroles`, and `roles` use the local cached copies by default to reduce latency. Set `ENTITY_REFERENCE_SOURCE=live` to read those five collections live instead. While Jalahalli has temporary local Upavasati corrections, set `ENTITY_LOCAL_HIERARCHY_OVERRIDE_NAMES=JALAHALLI`; the daily sync preserves only that Nagar and its descendants in `entities`/`parententities`, while all other Nagars and all `ssdatas`/`sanghdatas` records continue syncing. Clear the names variable after the developer data is corrected. The legacy `ENTITY_LOCAL_HIERARCHY_OVERRIDE=true` preserves the entire hierarchy. If the live connection is unavailable, all collections fall back to the local copies. The source entity database is never written. The interval is controlled by `ENTITY_CACHE_SYNC_INTERVAL_MINUTES` (default `1440`; set to `5` for rapid sync testing).
