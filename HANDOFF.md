...
The seven entity collections are periodically copied into the application database as a fallback cache. `ssdatas` and `sanghdatas` use `ENTITY_MONGO_URI` by default because person data may change. `entities`, `parententities`, `stharas`, `userroles`, and `roles` use the local cached copies by default to reduce latency. Set `ENTITY_REFERENCE_SOURCE=live` to read those five collections live instead. Set `ENTITY_LOCAL_HIERARCHY_OVERRIDE=true` to force `entities` and `parententities` to remain local and prevent their sync overwrite while temporary Upavasati corrections are maintained. If the live connection is unavailable, all collections fall back to the local copies. The source entity database is never written. The interval is controlled by `ENTITY_CACHE_SYNC_INTERVAL_MINUTES` (default `1440`; set to `5` for rapid sync testing).
