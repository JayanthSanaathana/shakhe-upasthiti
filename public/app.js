...
/**
 * When true, collapsed toggle columns are omitted entirely (no blank placeholder).
 * Toggle button sits beside Exclude Sunday Sanghik.
 */
let hideBlankToggleSlots = false;
    ...
    if (nagaraListContext) {
      ...
      if (nagaraListContext.mode === 'shakhe-status-split' && nagaraListContext.entityId) {
        return openShakheStatusSplit({
          ...
          attendanceDate: nagaraListContext.attendanceDate || '',
          saptahaMode: nagaraListContext.saptahaMode || '',
          upavasatiId: nagaraListContext.upavasatiId || '',
        });
      }
      if (
...
function logoutLocal() {
  ...
  hideBlankToggleSlots = false;
  const blankBtn = document.getElementById('nagara-hide-blank-columns');
  if (blankBtn) {
    blankBtn.setAttribute('aria-pressed', 'false');
    blankBtn.textContent = 'ಖಾಲಿ ಕಾಲಮ್‌ಗಳನ್ನು ಮರೆಮಾಡಿ/Hide blank columns';
  }
...
function returnToNagaraListContext() {
  ...
  if (ctx.mode === 'shakhe-status-split' && ctx.entityId && ctx.entityLevel) {
    return openShakheStatusSplit({
      ...
      saptahaMode: ctx.saptahaMode || '',
      upavasatiId: ctx.upavasatiId || '',
    });
  }
  ...
  if (nagaraListContext && nagaraListContext.mode === 'shakhe-varadi') {
    ...
    if (ret.mode === 'shakhe-status-split' && ret.entityId && ret.entityLevel) {
      return openShakheStatusSplit({
        ...
        attendanceDate: ret.attendanceDate || '',
        saptahaMode: ret.saptahaMode || '',
        upavasatiId: ret.upavasatiId || '',
      });
    }
...
const nagaraHideBlankColumnsEl = document.getElementById('nagara-hide-blank-columns');
if (nagaraHideBlankColumnsEl) {
  nagaraHideBlankColumnsEl.addEventListener('click', () => {
    hideBlankToggleSlots = !hideBlankToggleSlots;
    nagaraHideBlankColumnsEl.setAttribute('aria-pressed', hideBlankToggleSlots ? 'true' : 'false');
    nagaraHideBlankColumnsEl.textContent = hideBlankToggleSlots
      ? 'ಕಾಲಮ್‌ಗಳನ್ನು ತೋರಿಸಿ/Show column toggles'
      : 'ಖಾಲಿ ಕಾಲಮ್‌ಗಳನ್ನು ಮರೆಮಾಡಿ/Hide blank columns';
    if (nagaraReportKind === 'shakhe' && nagaraReportCache) {
      paintNagaraShakheVaradi(nagaraReportCache);
    } else {
...
function reportRowChild(row) {
  if (!row) return null;
  return row.upavasati || row.vasati || row.nagar || row.bhag || row.vibhag || null;
}
...
function shakheStatusCountLink(count, filter, entityLevel, entityId, entityName, vasatiId, upavasatiId) {
  const n = count || 0;
  ...
  return (
    ...
    `data-vasati-id="${escapeHtml(vasatiId || '')}" ` +
    `data-upavasati-id="${escapeHtml(upavasatiId || '')}">${n}</button>`
  );
}
...
function bindNagaraReportListClicks(table) {
  table.querySelectorAll('button.num-link[data-list-kind]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      const filter = btn.getAttribute('data-list-filter') || 'all';
      if (kind === 'vasati-report') {
        openNagaraVasatiReport(vasatiId, title, nagaraReportKind);
        return;
      }
      if (kind === 'upavasati') {
        openNagaraUpavasatiList(vasatiId, title, filter);
        return;
      }
  ...
  table.querySelectorAll('button.shakhe-status-split-link').forEach((btn) => {
    btn.addEventListener('click', () => {
      openShakheStatusSplit({
        ...
        upavasatiId: btn.getAttribute('data-upavasati-id') || '',
...
async function openNagaraVasatiReport(vasatiId, titleName, kind) {
  if (!vasatiId || !scopedNagarId()) return;
  varadiDrillStack.push({
    level: reportScopeLevel,
    entityId: reportScopeEntityId,
    entityName: reportScopeEntityName,
  ...
  try {
    ...
    if (range.excludeSunday) params.set('excludeSunday', '1');
    const program = kind === 'boudhik' || kind === 'sharirik';
    const url = program ? '/api/nagara/program-varadi' : '/api/nagara/shakhe-varadi';
    if (program) {
...
function dailyShakheCountLink(count, date, opts) {
  const n = Number(count) || 0;
  if (!n || !opts || !opts.entityId || !opts.entityLevel) return String(n);
  return `<button type="button" class="num-link shakhe-daily-link" ` +
    `data-date="${escapeHtml(date)}" data-entity-level="${escapeHtml(opts.entityLevel)}" ` +
    `data-entity-id="${escapeHtml(opts.entityId)}" data-entity-name="${escapeHtml(opts.entityName || '')}" ` +
    `data-vasati-id="${escapeHtml(opts.vasatiId || '')}" ` +
    `data-upavasati-id="${escapeHtml(opts.upavasatiId || '')}">${n}</button>`;
}

/** Week summary inside Shakha Saptaha: ran ≥1 day, or ran 0 days (21–27). */
function saptahaWeekCountLink(count, mode, opts) {
  const n = Number(count) || 0;
  if (!n || !opts || !opts.entityId || !opts.entityLevel) return String(n);
  return (
    `<button type="button" class="num-link shakhe-saptaha-link" ` +
    `data-saptaha-mode="${escapeHtml(mode)}" ` +
    `data-entity-level="${escapeHtml(opts.entityLevel)}" ` +
    `data-entity-id="${escapeHtml(opts.entityId)}" ` +
    `data-entity-name="${escapeHtml(opts.entityName || '')}" ` +
    `data-vasati-id="${escapeHtml(opts.vasatiId || '')}" ` +
...
function dailyShakheDates(data) {
  return Array.isArray(data.dailyDates) ? data.dailyDates : [];
}
...
function paintNagaraShakheVaradi(data) {
  ...
  const dailyDates = dailyShakheDates(data);
  ...
  const hideBlanks = Boolean(hideBlankToggleSlots);
  const showYojitaSlot = yojitaOpen || !hideBlanks;
  const showRunningSlot = runningOpen || !hideBlanks;
  ...
  const dailyCells = (rowOrCounts, opts) => {
    ...
    if (!dailyOpen) return '<td class="num"></td>';
  ...
  const daysGroupHead = !showDaysRanSlot
    ? ''
    : daysOpen
  ...
  const body = (data.rows || [])
    .map((row) => {
      ...
      if (isVasatiReport) {
        ...
        nameCell =
          `<button type="button" class="num-link vasati-name-link" data-list-kind="vasati-report" ` +
      ...
      const yojita = isLeaf
        ...
        : nextLevel && cid
          ...
          : cell(row.yojitaShakheCount);
      const upavasatiListAttrs = isLeaf && reportVasatiId
        ? ` data-list-kind="upavasati" data-vasati-id="${escapeHtml(reportVasatiId)}" data-list-title="${escapeHtml(title)}"`
        : '';
      ...
      const withoutShakhe = isLeaf && cid && row.upavasatiWithoutShakheCount > 0
        ? `<button type="button" class="num-link"${upavasatiListAttrs} data-list-filter="without-shakhe">${row.upavasatiWithoutShakheCount}</button>`
        : nextLevel && cid
          ? upavasatiScopeCountLink(row.upavasatiWithoutShakheCount, nextLevel, cid, title, 'without-shakhe')
          : cell(row.upavasatiWithoutShakheCount);
      ...
      const daysOpts = isLeaf
        ? {
          ...
          entityName: reportScopeEntityName || nagaraName || '',
          vasatiId: reportVasatiId,
          upavasatiId: isVasatiReport ? cid : '',
        }
        : cid
          ? {
      ...
      return (
        ...
        (showYojitaSlot ? (yojitaOpen ? `<td class="num">${yojita}</td>` : '<td class="num"></td>') : '') +
        (showRunningSlot ? (runningOpen ? `<td class="num">${running}</td>` : '<td class="num"></td>') : '') +
        dailyCells(row, daysOpts && { ...daysOpts, entityName: title }) +
        (showDaysRanSlot ? (daysOpen ? daysCells : `<td class="num days-ran-placeholder">·</td>`) : '') +
        (showNadayadaSlot ? (nadayadaOpen ? `<td class="num">${notRunning}</td>` : '<td class="num"></td>') : '') +
  ...
  const totDaysCells = !showDaysRanSlot
    ? ''
    : daysOpen
      ? Array.from({ length: dayCount + 1 }, (_, i) => {
  ...
  // Keep colspan in sync with rendered cells.
  const colCount =
    1 + // name
  ...
  const foot =
    ...
    (showYojitaSlot ? (yojitaOpen ? `<td class="num">${cell(tot.yojitaShakheCount)}</td>` : '<td class="num"></td>') : '') +
    (showRunningSlot ? (runningOpen ? `<td class="num">${cell(tot.nadayuthiruvaShakheCount)}</td>` : '<td class="num"></td>') : '') +
    dailyCells(tot, {
      entityLevel: level,
      entityId: reportScopeEntityId || nagaraId || '',
      entityName: reportScopeEntityName || nagaraName || '',
    ...
    totDaysCells +
    (showNadayadaSlot ? (nadayadaOpen ? `<td class="num">${cell(tot.nadayadaShakheCount)}</td>` : '<td class="num"></td>') : '') +
  ...
  table.querySelectorAll('[data-daily-toggle]').forEach((btn) => {
    btn.addEventListener('click', () => {
      shakheDailyColumnsOpen = !shakheDailyColumnsOpen;
      paintNagaraShakheVaradi(data);
    });
  });
  table.querySelectorAll('button.shakhe-daily-link').forEach((btn) => {
    btn.addEventListener('click', () => openShakheStatusSplit({
      attendanceDate: btn.getAttribute('data-date'),
      entityLevel: btn.getAttribute('data-entity-level'),
      entityId: btn.getAttribute('data-entity-id'),
      entityName: btn.getAttribute('data-entity-name') || '',
      vasatiId: btn.getAttribute('data-vasati-id') || '',
  ...
  table.querySelectorAll('button.shakhe-saptaha-link').forEach((btn) => {
    btn.addEventListener('click', () => openShakheStatusSplit({
      saptahaMode: btn.getAttribute('data-saptaha-mode') || 'ran',
      entityLevel: btn.getAttribute('data-entity-level'),
      entityId: btn.getAttribute('data-entity-id'),
      entityName: btn.getAttribute('data-entity-name') || '',
      vasatiId: btn.getAttribute('data-vasati-id') || '',
      upavasatiId: btn.getAttribute('data-upavasati-id') || '',
      itemFilter: btn.getAttribute('data-saptaha-mode') === 'noran' ? 'no' : 'yes',
    }));
  });
...
function paintDailyShakheListTable(shakhes) {
  const sorted = sortProgramSplitShakhes(shakhes);
  const visibleFields = shakheHierarchyVisibleFields(sorted,
    hierarchyKeysForScopeLevel(nagaraListContext && nagaraListContext.entityLevel));
  const keys = visibleFields.map((field) => field.key);
  const display = shakhesToHierDisplayRows(sorted, keys);
  const spans = programSplitRowspans(display, keys);
  const metrics = [
    ['taruna', 'ತರುಣ/Taruna'], ['balaka', 'ಬಾಲಕ/Balaka'], ['total', 'ಒಟ್ಟು/Total'],
    ['shishu', 'ಶಿಶು/Shishu'], ['mataBhagi', 'ಮಾತಾ-ಭಗಿನಿ/Mata Bhagini'],
  ];
  const heads = visibleFields.map((field) => `<th>${stackedLabel(field.label)}</th>`).join('') +
    `<th>${stackedLabel('ಶಾಖೆ/Shakhe')}</th><th>${stackedLabel('ಸಮಯ/Time')}</th>` +
    metrics.map(([, label]) => `<th class="num">${stackedLabel(label)}</th>`).join('');
  const rows = display.map((row, index) => {
    const s = row._shakhe;
    const attendance = s.upasthiti || {};
    const time = [TIMING_LABEL[s.timing] || s.timing, s.time].filter(Boolean).join(' · ') || '—';
    return `<tr>${shakheGroupedHierCells(display, spans, keys, index)}` +
      `<td>${escapeHtml(s.name || '—')}</td><td>${escapeHtml(time)}</td>` +
      metrics.map(([key]) => `<td class="num">${escapeHtml(String(attendance[key] ?? '—'))}</td>`).join('') + '</tr>';
  }).join('');
  const totals = metrics.map(([key]) => `<td class="num">${sorted.reduce((sum, s) => sum + Number((s.upasthiti || {})[key] || 0), 0)}</td>`).join('');
  return `<div class="table-wrap"><table class="varadi-table shakhe-list-table shakhe-yojita-table">` +
    `<thead><tr>${heads}</tr></thead><tbody>${rows || `<tr><td colspan="${keys.length + 7}">ಶಾಖೆಗಳಿಲ್ಲ/No shakhes</td></tr>`}</tbody>` +
    (sorted.length ? `<tfoot><tr class="report-total-row"><td colspan="${keys.length + 2}">${stackedLabel('ಒಟ್ಟು/Total')}</td>${totals}</tr></tfoot>` : '') +
    '</table></div>';
}
...
function paintShakheStatusSplitBody() {
  ...
  const attendanceDate = nagaraListContext.attendanceDate || '';
  const saptahaMode = nagaraListContext.saptahaMode || '';
  const showHierFilters = Boolean(attendanceDate) || Boolean(saptahaMode) || filter !== 'yes';
  ...
  let tableHtml;
  if (attendanceDate) {
    summary = `<div class="list-summary"><div class="list-summary-item">` +
      `${stackedLabel('ನಡೆದ ಶಾಖೆಗಳು/Nadeda Shakhegalu')} <strong>${filtered.length}</strong></div>` +
      `<div class="list-summary-item">${stackedLabel('ಉಪಸ್ಥಿತಿ/Upasthiti')} — ${escapeHtml(formatDateDisplay(attendanceDate))}</div></div>`;
...
async function openShakheStatusSplit(opts) {
  ...
  const entityName = (opts && opts.entityName) || '';
  const attendanceDate = (opts && opts.attendanceDate) || '';
  const saptahaMode = (opts && opts.saptahaMode) || '';
  if (attendanceDate && !/^\d{4}-\d{2}-\d{2}$/.test(attendanceDate)) return;
  if (saptahaMode && saptahaMode !== 'ran' && saptahaMode !== 'noran') return;
  const keepFilter =
  ...
  const itemFilter = saptahaMode
    ? saptahaMode === 'noran' ? 'no' : 'yes'
    : attendanceDate
      ? 'yes'
      : keepFilter === 'yojita' ? 'all' : keepFilter;
  nagaraListContext = {
    ...
    entityName,
    vasatiId: (opts && opts.vasatiId) || null,
    upavasatiId: (opts && opts.upavasatiId) || null,
  ...
  setNagaraListLoading(true);
  try {
    const range = saptahaMode
      ? { from: '2026-09-21', to: '2026-09-27', excludeSunday: false }
      : attendanceDate
        ? { from: attendanceDate, to: attendanceDate, excludeSunday: false }
        : nagaraVaradiRangeDays();
    const params = new URLSearchParams({
      level: entityLevel,
      entityId,
      from: range.from,
    ...
    if (opts && opts.upavasatiId) params.set('upavasatiId', opts.upavasatiId);
    if (range.excludeSunday) params.set('excludeSunday', '1');
...
async function openShakheDaysRanList(opts) {
  ...
  try {
    ...
    if (vasatiId) params.set('vasatiId', vasatiId);
    if (nagaraListContext.upavasatiId) params.set('upavasatiId', nagaraListContext.upavasatiId);
    if (range.excludeSunday) params.set('excludeSunday', '1');
    const res = await fetch(`/api/varadi/shakhe-status-shakhes?${params.toString()}`);
    const data = await res.json().catch(() => ({}));
    if (bounceIfVaradiAuth(res, data)) return;
