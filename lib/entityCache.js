...
async function syncEntityCacheIfNeeded(sourceDb, targetDb) {
  ...
  const localWasKept = previous && previous.counts &&
    (previous.counts.entities === 'kept-local' || previous.counts.parententities === 'kept-local');
  if (elapsed < syncIntervalMinutes() * 60 * 1000 && !(localWasKept && !keepLocalHierarchy())) {
    return { skipped: true, syncedAt: previous.syncedAt };
  }

  const counts = {};
  for (const name of COLLECTIONS) {
    if (keepLocalHierarchy() && (name === 'entities' || name === 'parententities')) {
      counts[name] = 'kept-local';
      continue;
    }
    counts[name] = await copyCollection(sourceDb, targetDb, name);
  }
