...
async function validatedFields(body) {
  // Empty sankhye → 0; at least one of the four must be > 0.
  ...
  if (
    !(taruna.value > 0 || balaka.value > 0 || shishu.value > 0 || mataBhagi.value > 0)
  ) {
    return {
      error: 'Enter at least one of Taruna, Balaka, Shishu, or Mata Bhagini greater than 0',
    };
  }
  ...
  for (const raw of rawPravasiList) {
    ...
    let otherResponsibility = clipText(raw.otherResponsibility, 120) || null;
    ...
    if (phone && (!name || !responsibility || !otherResponsibility || !personId)) {
      ...
      if (match) {
        ...
        if (!otherResponsibility && match.otherResponsibility) {
          otherResponsibility = clipText(match.otherResponsibility, 120) || null;
        }
...
function serializePersonSnap(person) {
  ...
  return {
    ...
    otherResponsibility: person.otherResponsibility || null,
...
async function enrichPravasiSnap(person) {
  ...
  if (!snap) return null;
  if (snap.name && (snap.responsibility || snap.otherResponsibility)) return snap;
  ...
  return {
    ...
    otherResponsibility: snap.otherResponsibility || match.otherResponsibility || null,
