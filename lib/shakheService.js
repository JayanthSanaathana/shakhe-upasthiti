...
const DeletedShakhe = require('../models/DeletedShakhe');
...
/** Classify clock time: [04:00,11:00) prabhat, [11:00,16:00) madhyana, [16:00,19:00) sayam, else ratri. */
...
async function validatedFields(body) {
  ...
  if (!TIMING.includes(timing)) return { error: 'Time is invalid' };

  const shakheType = String(body.shakheType || '').trim();
  if (!SHAKHE_TYPE.includes(shakheType)) return { error: 'Select shakhe type' };

  const mukha = requirePhone(body.mukhashikshakPhone, 'Mukhya Shikshak phone');
...
function serialize(shakhe) {
  ...
  for (const level of CHAIN) {
    path[level.key] = {
      ...
      name: displayEntityName(shakhe[level.key].name),
    };
  }
  return {
    ...
    reportHideIntervals: Array.isArray(shakhe.reportHideIntervals)
      ? shakhe.reportHideIntervals.map((iv) => ({
          from: iv.from || null,
          to: iv.to == null || iv.to === '' ? null : iv.to,
        }))
      : [],
    reportHidden: isCurrentlyHidden(shakhe.reportHideIntervals),
  };
}

async function canManageShakheAtScope(shakhe, level, entityId) {
  if (!shakhe || !isObjectId(entityId)) return false;
  const scopeKey = { vibhag: 'vibhag', bhag: 'bhag', nagara: 'nagar' }[level];
  if (scopeKey) {
    const ref = shakhe[scopeKey] && shakhe[scopeKey].entity;
    return Boolean(ref && String(ref) === String(entityId));
  }
  if (level === 'prant') {
    const vibhagId = shakhe.vibhag && shakhe.vibhag.entity;
    return Boolean(vibhagId && (await isDescendantOrSelf(vibhagId, entityId)));
  }
  return false;
}

async function setReportHidden(id, level, entityId, hidden, onDate) {
  if (!isObjectId(id)) return { error: 'Invalid shakhe' };
  const shakhe = await Shakhe.findById(id);
  if (!shakhe) return { error: 'Shakhe not found', status: 404 };
  if (!(await canManageShakheAtScope(shakhe, level, entityId))) {
    return { error: 'Not allowed for this entity', status: 403 };
  }
  const day = String(onDate || todayIst());
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) return { error: 'Invalid date' };
  const current = Array.isArray(shakhe.reportHideIntervals) ? shakhe.reportHideIntervals : [];
  shakhe.reportHideIntervals = hidden
    ? openHideInterval(current, day)
    : closeHideInterval(current, day);
  shakhe.updatedAt = new Date();
  await shakhe.save();
...
/** Load shakhe for any varadi session (access checked by the route). */
async function viewForVaradi(id) {
  if (!isObjectId(id)) return { error: 'Invalid shakhe' };
  const shakhe = await Shakhe.findById(id);
  if (!shakhe) return { error: 'Shakhe not found', status: 404 };
  const [row] = await withPeopleNames([serialize(shakhe)]);
  return { shakhe: row, nagarId: shakhe.nagar && shakhe.nagar.entity ? String(shakhe.nagar.entity) : '' };
}
...
async function listForScope(level, entityId) {
  ...
  let query;
  if (level === 'nagara') query = { 'nagar.entity': entityId };
  else if (level === 'bhag') query = { 'bhag.entity': entityId };
  else if (level === 'vibhag') query = { 'vibhag.entity': entityId };
  else {
    const vibhags = await descendantsOfSthara(entityId, 'Vibhag');
    query = { 'vibhag.entity': { $in: vibhags.map((row) => row.id) } };
  }

  const rows = await Shakhe.find(query).sort({ createdAt: -1 }).lean();
  const serialized = rows.map(serialize);
  // Upper lists may contain thousands of Shakhes and do not display bearer
  // names. Resolve missing names only for the smaller Nagara list.
  return { shakhes: level === 'nagara' ? await withPeopleNames(serialized) : serialized };
}

/** Archive a Shakhe and all of its daily data before removing active records. */
async function archiveAndDeleteShakhe(id, level, entityId, meta) {
  if (!isObjectId(id)) return { error: 'Invalid shakhe' };
  if (!isObjectId(entityId)) return { error: 'Invalid entity' };

  const shakhe = await Shakhe.findById(id).lean();
  if (!shakhe) return { error: 'Shakhe not found', status: 404 };
  if (!(await canManageShakheAtScope(shakhe, level, entityId))) {
    return { error: 'Not allowed for this entity', status: 403 };
  }

  const upasthitis = await ShakheUpasthiti.find({ shakhe: shakhe._id })
    .sort({ date: 1 })
    .lean();
  const archiveFields = {
    originalShakheId: shakhe._id,
    shakhe,
    upasthitis,
    upasthitiCount: upasthitis.length,
    deletedAt: new Date(),
    deletedIp: (meta && meta.ip) || null,
    deletedByLevel: level,
    deletedByEntity: entityId,
    archiveVersion: 1,
  };

  let archive;
  try {
    archive = await DeletedShakhe.findOneAndUpdate(
      { originalShakheId: shakhe._id },
      { $setOnInsert: archiveFields },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );
  } catch (err) {
    // Two delete clicks may race on the unique archive ID. Reuse the first,
    // complete archive instead of risking removal without a backup.
    if (!err || err.code !== 11000) throw err;
    archive = await DeletedShakhe.findOne({ originalShakheId: shakhe._id });
  }
  if (!archive) return { error: 'Could not archive shakhe', status: 500 };

  // Remove the parent first so new entry requests can no longer resolve this
  // Shakhe while its already-archived child records are being cleared.
  await Shakhe.deleteOne({ _id: shakhe._id });
  await ShakheUpasthiti.deleteMany({ shakhe: shakhe._id });

  return {
    deleted: true,
    shakheId: String(shakhe._id),
    upasthitiCount: Number(archive.upasthitiCount) || 0,
  };
}
