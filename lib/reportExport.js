...
function dailyNadedaHeaders() {
  return [
    '21–27 Ondu Dina Nadeda',
    ...DAILY_REPORT_DATES.map((date) => `Shakha Saptaha ${Number(date.slice(8))} (${date})`),
    '21–27 Nadayada Shakhegalu',
  ];
}
...
function dailyNadedaValues(rowOrCounts) {
  ...
  return [
    ran,
    ...DAILY_REPORT_DATES.map((date) => counts[date] || 0),
    nadayada,
  ];
}
...
function applyNagaraFormulas(scope, ws, meta, options) {
  ...
  // After Running (col 6): week ran + 7 daily + week nadayada, then 0..N days-ran buckets.
  const daysRanStart = 7 + DAILY_NADEDA_COUNT + 2;
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    for (let column = 2; column <= ws.columnCount; column += 1) {
      ...
      if (options.kind === 'shakhe' && scope.level === 'vibhag') {
        ...
        const averageStart = 9 + DAILY_NADEDA_COUNT + 2 + dayCount;
...
async function buildWorkbook(rootLevel, rootId, options) {
  ...
  for (const scope of tree.scopes) {
    ...
    finishSheet(ws);
    if (options.kind === 'shakhe') {
      // 0..N "days ran" buckets sit after Shakha Saptaha (week + 7 days + week nadayada).
      const daysRanStart = 7 + DAILY_NADEDA_COUNT + 2;
...
function pdfBuffer(root, options) {
  return new Promise((resolve, reject) => {
    ...
    const dailyHeads = [
      '21–27 Ran',
      ...DAILY_REPORT_DATES.map((date) => `Saptaha ${Number(date.slice(8))}`),
      '21–27 Nadayada',
    ];
