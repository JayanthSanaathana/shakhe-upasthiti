...
function standardColumns(dayCount) {
  ...
  return [LEVEL_LABEL.vasati, 'Total Upavasati', 'Upavasati with Shakhe', 'Upavasatis without Shakhe', 'Yojita Shakhe', 'Running Shakhe', ...days, 'Not running', 'Avg Taruna', 'Avg Balaka', 'Avg Total', 'Avg Shishu', 'Avg Mata Bhagini', 'Contacted homes', 'Contacted people'];
}
...
function applyNagaraFormulas(scope, ws, meta, options) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (options.kind === 'shakhe') {
      ...
      for (let days = 0; days <= dayCount; days += 1) {
        replaceWithFormula(ws.getCell(row, 7 + days), countFormula(daysRanRange, row, days, false));
      }
      const notRunningCol = 8 + dayCount;
      replaceWithFormula(ws.getCell(row, notRunningCol), `E${row}-F${row}`);
      const averageStart = 9 + dayCount;
  ...
  if (options.kind === 'shakhe') {
    const dayCount = meta.calendarDayCount;
    for (let days = 0; days <= dayCount; days += 1) {
      replaceWithFormula(ws.getCell(totalRow, 7 + days), countFormula(daysRanRange, totalRow, days, true));
    }
    const notRunningCol = 8 + dayCount;
...
async function buildWorkbook(rootLevel, rootId, options) {
  ...
  for (const scope of tree.scopes) {
    ...
    finishSheet(ws);
    if (options.kind === 'shakhe') {
      // 0..N "days ran" buckets begin at column G. Keep them available but
      // hidden by default; Excel/Google Sheets users can unhide when needed.
      for (let column = 7; column <= 7 + calendarDayCount; column += 1) {
...
function pdfBuffer(root, options) {
  return new Promise((resolve, reject) => {
    ...
    const cols = options.kind === 'shakhe'
      ? ['Unit', 'Total Upavasati', 'Upavasati with Shakhe', 'Upavasatis without Shakhe', 'Yojita', 'Running', 'Not running', 'Avg T', 'Avg B', 'Avg Total', 'Avg Shishu', 'Avg Mata', 'Homes', 'People']
    ...
    for (const row of data.rows || []) {
      ...
      const values = options.kind === 'shakhe'
        ? [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.upavasatiWithoutShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, row.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
    ...
    if (data.totals) {
      ...
      const values = options.kind === 'shakhe'
        ? ['Total', data.totals.upavasatiCount || 0, data.totals.upavasatiWithShakheCount || 0, data.totals.upavasatiWithoutShakheCount || 0, data.totals.yojitaShakheCount || 0, data.totals.nadayuthiruvaShakheCount || 0, data.totals.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
