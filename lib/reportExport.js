...
const SHARIRIK_LABELS = {
  suryanamaskar: 'ಸೂರ್ಯನಮಸ್ಕಾರ/Suryanamaskar', yogasana: 'ಯೋಗಾಸನ/Yogasana', samata: 'ಸಮತಾ/Samata', sanchalana: 'ಸಂಚಲನ/Sanchalana',
...
/** Fixed campaign week — same as on-screen Daily Nadeda Shakhegalu columns. */
const DAILY_REPORT_DATES = Array.from({ length: 7 }, (_, index) => `2026-09-${21 + index}`);
const DAILY_NADEDA_COUNT = DAILY_REPORT_DATES.length;

function dailyNadedaHeaders() {
  return DAILY_REPORT_DATES.map(
    (date) => `Shakha Saptaha ${Number(date.slice(8))} (${date})`
  );
}

function dailyNadedaValues(rowOrCounts) {
  const counts = rowOrCounts && rowOrCounts.dailyShakheCounts
    ? rowOrCounts.dailyShakheCounts
    : rowOrCounts || {};
  return DAILY_REPORT_DATES.map((date) => counts[date] || 0);
}
...
function standardColumns(dayCount) {
  ...
  return [
    LEVEL_LABEL.vasati,
    'Total Upavasati',
    'Upavasati with Shakhe',
    'Upavasatis without Shakhe',
    'Yojita Shakhe',
    'Running Shakhe',
    ...dailyNadedaHeaders(),
    ...days,
...
function standardValues(row, dayCount) {
  ...
  return [
    childOf(row, row._level)?.name || '',
    row.upavasatiCount || 0,
    row.upavasatiWithShakheCount || 0,
    row.upavasatiWithoutShakheCount || 0,
    row.yojitaShakheCount || 0,
    row.nadayuthiruvaShakheCount || 0,
    ...dailyNadedaValues(row),
    ...Array.from({ length: dayCount + 1 }, (_, i) => buckets[i] || 0),
    row.nadayadaShakheCount || 0,
    a.taruna ?? '',
    a.balaka ?? '',
    a.total ?? '',
    a.shishu ?? '',
...
function standardTotalValues(total, dayCount) {
  const a = total.averages || {};
  const s = total.ottuSamparka || {};
  const buckets = total.daysRanBuckets || [];
  return [
    'Total',
    total.upavasatiCount || 0,
    total.upavasatiWithShakheCount || 0,
    total.upavasatiWithoutShakheCount || 0,
    total.yojitaShakheCount || 0,
    total.nadayuthiruvaShakheCount || 0,
    ...dailyNadedaValues(total),
    ...Array.from({ length: dayCount + 1 }, (_, i) => buckets[i] || 0),
    total.nadayadaShakheCount || 0,
    a.taruna ?? '',
    a.balaka ?? '',
    a.total ?? '',
    a.shishu ?? '',
...
function programColumns(data) {
  return [LEVEL_LABEL.vasati, 'Total Upavasati', 'Upavasati with Shakhe', 'Upavasatis without Shakhe', 'Yojita Shakhe', 'Running Shakhe', 'Not running', ...(data.catalog || []).map((i) => `${i.kn}/${i.en}`)];
}

function programValues(row, data) {
  return [childOf(row, row._level)?.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.upavasatiWithoutShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0,
    row.nadayadaShakheCount || 0, ...(data.catalog || []).map((i) => (row.itemCounts || {})[i.id] || 0)];
}

function programTotalValues(total, data) {
  return ['Total', total.upavasatiCount || 0, total.upavasatiWithShakheCount || 0, total.upavasatiWithoutShakheCount || 0, total.yojitaShakheCount || 0, total.nadayuthiruvaShakheCount || 0,
...
function applyNagaraFormulas(scope, ws, meta, options) {
  ...
  // After Running (col 6): 7 fixed daily Nadeda cols, then 0..N days-ran buckets.
  const daysRanStart = 7 + DAILY_NADEDA_COUNT;
  for (let i = 0; i < rows.length; i += 1) {
    const row = i + 2;
    replaceWithFormula(ws.getCell(row, 5), countFormula(visibleDaysRange, row, '">0"', false));
    replaceWithFormula(ws.getCell(row, 6), countFormula(daysRanRange, row, '">0"', false));
    if (options.kind === 'shakhe') {
      const dayCount = meta.calendarDayCount;
      for (let days = 0; days <= dayCount; days += 1) {
        replaceWithFormula(
          ws.getCell(row, daysRanStart + days),
          countFormula(daysRanRange, row, days, false)
        );
      }
      const notRunningCol = daysRanStart + dayCount + 1;
      replaceWithFormula(ws.getCell(row, notRunningCol), `E${row}-F${row}`);
      const averageStart = notRunningCol + 1;
      ...
      replaceWithFormula(ws.getCell(row, 7), `E${row}-F${row}`);
      (data.catalog || []).forEach((item, offset) => {
        ...
        replaceWithFormula(ws.getCell(row, 8 + offset), countFormula(helperRange, row, criterion, false));
      });
    }
  }
  ...
  replaceWithFormula(ws.getCell(totalRow, 5), countFormula(visibleDaysRange, totalRow, '">0"', true));
  replaceWithFormula(ws.getCell(totalRow, 6), countFormula(daysRanRange, totalRow, '">0"', true));
  if (options.kind === 'shakhe') {
    const dayCount = meta.calendarDayCount;
    for (let days = 0; days <= dayCount; days += 1) {
      replaceWithFormula(
        ws.getCell(totalRow, daysRanStart + days),
        countFormula(daysRanRange, totalRow, days, true)
      );
    }
    const notRunningCol = daysRanStart + dayCount + 1;
    replaceWithFormula(ws.getCell(totalRow, notRunningCol), `E${totalRow}-F${totalRow}`);
    const averageStart = notRunningCol + 1;
    [9, 10, 11, 12, 13].forEach((dailyCol, offset) => {
    ...
    replaceWithFormula(ws.getCell(totalRow, 7), `E${totalRow}-F${totalRow}`);
    (data.catalog || []).forEach((item, offset) => {
      const helperRange = dailyRange(meta, meta.itemCountCols.get(`${options.kind}:${item.id}`));
      const criterion = `IF(${quotedSheet(meta.filtersName)}!$B$8="Happened",">0",${quotedSheet(meta.filtersName)}!$B$8)`;
      replaceWithFormula(ws.getCell(totalRow, 8 + offset), countFormula(helperRange, totalRow, criterion, true));
    });
  }
}
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    for (let column = 2; column <= ws.columnCount; column += 1) {
      ...
      if (options.kind === 'shakhe' && scope.level === 'vibhag') {
        ...
        const averageStart = 9 + DAILY_NADEDA_COUNT + dayCount;
...
async function buildWorkbook(rootLevel, rootId, options) {
  ...
  for (const scope of tree.scopes) {
    ...
    finishSheet(ws);
    if (options.kind === 'shakhe') {
      // 0..N "days ran" buckets sit after the fixed 21–27 daily Nadeda columns.
      const daysRanStart = 7 + DAILY_NADEDA_COUNT;
      for (let column = daysRanStart; column <= daysRanStart + calendarDayCount; column += 1) {
...
function pdfBuffer(root, options) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.6);
    const dailyHeads = DAILY_REPORT_DATES.map((date) => `ಸಪ್ತಾಹ ${Number(date.slice(8))}`);
    const cols = options.kind === 'shakhe'
      ? ['Unit', 'Total Upavasati', 'With Shakhe', 'Without', 'Yojita', 'Running', ...dailyHeads, 'Not running', 'Avg T', 'Avg B', 'Avg Total', 'Avg Shishu', 'Avg Mata', 'Homes', 'People']
      : ['Unit', 'Total Upavasati', 'Upavasati with Shakhe', 'Upavasatis without Shakhe', 'Yojita', 'Running', 'Not running', ...(data.catalog || []).map((i) => i.en)];
    const pageWidth = doc.page.width - 56; const widths = cols.map((_, i) => i === 0 ? Math.max(90, pageWidth * 0.14) : (pageWidth - Math.max(90, pageWidth * 0.14)) / (cols.length - 1));
    const drawRow = (values, header) => {
      ...
      values.forEach((v, i) => { const shown = pdfDisplayText(v); doc.rect(x, y, widths[i], height).fillAndStroke(header ? '#1F4E78' : '#FFFFFF', '#B4C7E7'); selectPdfFont(doc, shown).fillColor(header ? '#FFFFFF' : '#111111').fontSize(header ? 6 : 7).text(shown, x + 2, y + 5, { width: widths[i] - 4, height: height - 8, ellipsis: true }); x += widths[i]; });
      doc.y = y + height;
    };
    drawRow(cols, true);
    for (const row of data.rows || []) {
      ...
      const values = options.kind === 'shakhe'
        ? [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.upavasatiWithoutShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, ...dailyNadedaValues(row), row.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
        : [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.upavasatiWithoutShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, row.nadayadaShakheCount || 0, ...(data.catalog || []).map((i) => (row.itemCounts || {})[i.id] || 0)];
      drawRow(values, false);
    }
    if (data.totals) {
      const a = data.totals.averages || {}; const s = data.totals.ottuSamparka || {};
      const values = options.kind === 'shakhe'
        ? ['Total', data.totals.upavasatiCount || 0, data.totals.upavasatiWithShakheCount || 0, data.totals.upavasatiWithoutShakheCount || 0, data.totals.yojitaShakheCount || 0, data.totals.nadayuthiruvaShakheCount || 0, ...dailyNadedaValues(data.totals), data.totals.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
        : ['Total', data.totals.upavasatiCount || 0, data.totals.upavasatiWithShakheCount || 0, data.totals.upavasatiWithoutShakheCount || 0, data.totals.yojitaShakheCount || 0, data.totals.nadayuthiruvaShakheCount || 0, data.totals.nadayadaShakheCount || 0, ...(data.catalog || []).map((i) => (data.totals.itemCounts || {})[i.id] || 0)];
      drawRow(values, true);
    }
    doc.end();
  });
