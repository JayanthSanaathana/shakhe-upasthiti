...
function dailyNadedaHeaders() {
  return DAILY_REPORT_DATES.map(
    (date) => `${Number(date.slice(8))} Nadeda Shakhegalu (${date})`
  );
}

function dailyNadedaValues(counts) {
  const map = counts || {};
  return DAILY_REPORT_DATES.map((date) => map[date] || 0);
}
...
function standardValues(row, dayCount) {
  ...
  return [
    ...
    ...dailyNadedaValues(row.dailyShakheCounts),
...
function standardTotalValues(total, dayCount) {
  ...
  return [
    ...
    ...dailyNadedaValues(total.dailyShakheCounts),
    ...Array.from({ length: dayCount + 1 }, (_, i) => buckets[i] || 0),
...
async function buildWorkbook(rootLevel, rootId, options) {
  ...
  for (const scope of tree.scopes) {
    ...
    if (options.kind === 'shakhe') {
      ...
      // Keep buckets available but hidden by default.
      const daysRanStart = 7 + DAILY_NADEDA_COUNT;
...
function pdfBuffer(root, options) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.6);
    const dailyHeads = DAILY_REPORT_DATES.map((date) => `${Number(date.slice(8))} Nadeda`);
    ...
    for (const row of data.rows || []) {
      ...
      const values = options.kind === 'shakhe'
        ? [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.upavasatiWithoutShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, ...dailyNadedaValues(row.dailyShakheCounts), row.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
