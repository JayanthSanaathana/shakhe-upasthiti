...
function standardColumns(dayCount) {
  ...
  return [LEVEL_LABEL.vasati, 'Total Upavasati', 'Upavasati with Shakhe', 'Yojita Shakhe', 'Running Shakhe', ...days, 'Not running', 'Avg Taruna', 'Avg Balaka', 'Avg Total', 'Avg Shishu', 'Avg Mata Bhagini', 'Contacted homes', 'Contacted people'];
}

function standardValues(row, dayCount) {
  const a = row.averages || {}; const s = row.ottuSamparka || {}; const buckets = row.daysRanBuckets || [];
  return [childOf(row, row._level)?.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0,
...
function standardTotalValues(total, dayCount) {
  ...
  return ['Total', total.upavasatiCount || 0, total.upavasatiWithShakheCount || 0, total.yojitaShakheCount || 0, total.nadayuthiruvaShakheCount || 0,
    ...Array.from({ length: dayCount + 1 }, (_, i) => buckets[i] || 0), total.nadayadaShakheCount || 0,
    a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? ''];
}

function programColumns(data) {
  return [LEVEL_LABEL.vasati, 'Total Upavasati', 'Upavasati with Shakhe', 'Yojita Shakhe', 'Running Shakhe', 'Not running', ...(data.catalog || []).map((i) => `${i.kn}/${i.en}`)];
}
...
function applyNagaraFormulas(scope, ws, meta, options) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    const row = i + 2;
    replaceWithFormula(ws.getCell(row, 4), countFormula(visibleDaysRange, row, '">0"', false));
    replaceWithFormula(ws.getCell(row, 5), countFormula(daysRanRange, row, '">0"', false));
    if (options.kind === 'shakhe') {
      const dayCount = meta.calendarDayCount;
      for (let days = 0; days <= dayCount; days += 1) {
        replaceWithFormula(ws.getCell(row, 6 + days), countFormula(daysRanRange, row, days, false));
      }
      const notRunningCol = 7 + dayCount;
      replaceWithFormula(ws.getCell(row, notRunningCol), `D${row}-E${row}`);
      const averageStart = 8 + dayCount;
      ...
      replaceWithFormula(ws.getCell(row, 6), `D${row}-E${row}`);
      (data.catalog || []).forEach((item, offset) => {
        ...
        replaceWithFormula(ws.getCell(row, 7 + offset), countFormula(helperRange, row, criterion, false));
      });
    }
  }
  ...
  replaceWithFormula(ws.getCell(totalRow, 4), countFormula(visibleDaysRange, totalRow, '">0"', true));
  replaceWithFormula(ws.getCell(totalRow, 5), countFormula(daysRanRange, totalRow, '">0"', true));
  if (options.kind === 'shakhe') {
    const dayCount = meta.calendarDayCount;
    for (let days = 0; days <= dayCount; days += 1) {
      replaceWithFormula(ws.getCell(totalRow, 6 + days), countFormula(daysRanRange, totalRow, days, true));
    }
    const notRunningCol = 7 + dayCount;
    replaceWithFormula(ws.getCell(totalRow, notRunningCol), `D${totalRow}-E${totalRow}`);
    const averageStart = 8 + dayCount;
    [9, 10, 11, 12, 13].forEach((dailyCol, offset) => {
    ...
    replaceWithFormula(ws.getCell(totalRow, 6), `D${totalRow}-E${totalRow}`);
    (data.catalog || []).forEach((item, offset) => {
      const helperRange = dailyRange(meta, meta.itemCountCols.get(`${options.kind}:${item.id}`));
      const criterion = `IF(${quotedSheet(meta.filtersName)}!$B$8="Happened",">0",${quotedSheet(meta.filtersName)}!$B$8)`;
      replaceWithFormula(ws.getCell(totalRow, 7 + offset), countFormula(helperRange, totalRow, criterion, true));
    });
  }
}
...
async function buildWorkbook(rootLevel, rootId, options) {
  ...
  for (const scope of tree.scopes) {
    ...
    finishSheet(ws);
    if (options.kind === 'shakhe') {
      // 0..N "days ran" buckets begin at column F. Keep them available but
      // hidden by default; Excel/Google Sheets users can unhide when needed.
      for (let column = 6; column <= 6 + calendarDayCount; column += 1) {
...
function pdfBuffer(root, options) {
  return new Promise((resolve, reject) => {
    ...
    const cols = options.kind === 'shakhe'
      ? ['Unit', 'Total Upavasati', 'Upavasati with Shakhe', 'Yojita', 'Running', 'Not running', 'Avg T', 'Avg B', 'Avg Total', 'Avg Shishu', 'Avg Mata', 'Homes', 'People']
      : ['Unit', 'Total Upavasati', 'Upavasati with Shakhe', 'Yojita', 'Running', 'Not running', ...(data.catalog || []).map((i) => i.en)];
    ...
    for (const row of data.rows || []) {
      ...
      const values = options.kind === 'shakhe'
        ? [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, row.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
        : [child && child.name || '', row.upavasatiCount || 0, row.upavasatiWithShakheCount || 0, row.yojitaShakheCount || 0, row.nadayuthiruvaShakheCount || 0, row.nadayadaShakheCount || 0, ...(data.catalog || []).map((i) => (row.itemCounts || {})[i.id] || 0)];
      drawRow(values, false);
    }
    if (data.totals) {
      const a = data.totals.averages || {}; const s = data.totals.ottuSamparka || {};
      const values = options.kind === 'shakhe'
        ? ['Total', data.totals.upavasatiCount || 0, data.totals.upavasatiWithShakheCount || 0, data.totals.yojitaShakheCount || 0, data.totals.nadayuthiruvaShakheCount || 0, data.totals.nadayadaShakheCount || 0, a.taruna ?? '', a.balaka ?? '', a.total ?? '', a.shishu ?? '', a.mataBhagi ?? '', s.manegalu ?? '', s.vyaktigalu ?? '']
        : ['Total', data.totals.upavasatiCount || 0, data.totals.upavasatiWithShakheCount || 0, data.totals.yojitaShakheCount || 0, data.totals.nadayuthiruvaShakheCount || 0, data.totals.nadayadaShakheCount || 0, ...(data.catalog || []).map((i) => (data.totals.itemCounts || {})[i.id] || 0)];
      drawRow(values, true);
    }
    doc.end();
  });
