...
function serializeEntity(entity) {
  ...
  // Live ID + optional Jalahalli local-name overlay (display only).
  const overlay = require('./jalahalliNameOverlay');
  const rawName = overlay.overlayName(id, entity.name);
  return { id, name: displayEntityName(rawName) };
}
...
async function descendantsOfStharaLive(rootId, targetSthara) {
  await require('./jalahalliNameOverlay').ensureOverlayMap();
...
async function ancestorsByStharaLive(entityId) {
  const overlay = require('./jalahalliNameOverlay');
  await overlay.ensureOverlayMap();
  const byName = {};
  let frontier = [entityId.toString()];
  const seen = new Set(frontier);
  for (let depth = 0; depth < 20 && frontier.length; depth += 1) {
    const entities = await Entity.find({ _id: { $in: frontier } }).select('name sthara');
    for (const entity of entities) {
      const level = await stharaName(entity);
      if (!level || byName[level]) continue;
      const overName = overlay.overlayName(entity._id.toString(), entity.name);
      byName[level] = {
        _id: entity._id,
        name: overName,
        sthara: entity.sthara,
      };
    }
    const edges = await ParentEntity.find({ currentEntity: { $in: frontier } })
      .select('parentEntity')
...
/** Carry each actual parent path downwards; never stitch unrelated ancestors together. */
async function upavasatiPathsUnder(rootId) {
  await require('./jalahalliNameOverlay').ensureOverlayMap();
  const root = await Entity.findById(rootId).lean();
  if (!root) return [];
  const rootLevel = await stharaName(root);
  const levels = ['Prant', ...CHAIN.map((entry) => entry.sthara)];
  const keys = { Vibhag: 'vibhag', Bhag: 'bhag', Nagar: 'nagar', Vasati: 'vasati' };
  const start = levels.indexOf(rootLevel);
  if (start < 0 || start >= levels.length - 1) return [];
  const initial = {};
  // Ancestors above the selected scope are labels only. Below it, every field
  // comes from the same downward path, including for multi-parent children.
  const ancestors = await ancestorsByStharaForIds([String(rootId)]);
  for (const [label, key] of Object.entries(keys)) {
    if (levels.indexOf(label) < start) initial[key] = (ancestors.get(String(rootId)) || {})[label] || null;
  }
  if (keys[rootLevel]) initial[keys[rootLevel]] = serializeEntity(root);
  let frontier = [{ id: String(rootId), placed: initial }];
  for (let index = start + 1; index < levels.length; index += 1) {
    const edges = await ParentEntity.find({ parentEntity: { $in: [...new Set(frontier.map((p) => p.id))] } })
      .select('parentEntity currentEntity').lean();
    const target = await stharaId(levels[index]);
    const children = await Entity.find({
      _id: { $in: [...new Set(edges.map((e) => String(e.currentEntity)))] }, sthara: target,
    }).select('name sthara').lean();
    const byId = new Map(children.map((child) => [String(child._id), serializeEntity(child)]));
    const childrenByParent = new Map();
    for (const edge of edges) {
      const parent = String(edge.parentEntity);
      if (!childrenByParent.has(parent)) childrenByParent.set(parent, new Set());
      childrenByParent.get(parent).add(String(edge.currentEntity));
    }
    const next = [];
    for (const path of frontier) {
      for (const childId of childrenByParent.get(path.id) || []) {
        const child = byId.get(childId);
        if (!child) continue;
        const placed = { ...path.placed };
        if (keys[levels[index]]) placed[keys[levels[index]]] = child;
        next.push({ id: child.id, name: child.name, placed });
      }
    }
    frontier = next;
    ...
    for (let depth = 0; depth < 20 && frontier.length; depth += 1) {
      ...
      for (const parent of parents) {
        ...
        if (label && !ancestors[label]) {
          ...
          ancestors[label] = {
            id,
            name: displayEntityName(overlay.overlayName(id, parent.name)),
          };
        }
