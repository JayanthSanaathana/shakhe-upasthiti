...
async function loadGramsByVasati(nagarId) {
  ...
  for (const edge of edges) {
    ...
    map.set(id, { id, name: displayEntityName(child.name) });
  }
...
function placedFromShakhe(shakhe) {
  return {
...
function placedFromAncestors(ancestors) {
  return {
    vibhag: ancestors.Vibhag || null,
    bhag: ancestors.Bhag || null,
    nagar: ancestors.Nagar || null,
    vasati: ancestors.Vasati || null,
  };
}
...
async function listUpavasatisForScope({ level, entityId, filter }) {
  ...
  const scopeSthara = { prant: 'Prant', vibhag: 'Vibhag', bhag: 'Bhag', nagara: 'Nagar' }[level];
  ...
  // with-shakhe only needs shakhe-backed rows; skip the full upavasati walk.
  const upavasatisPromise =
    kind === 'with-shakhe'
      ? Promise.resolve(null)
      : hierarchy.descendantsOfSthara(entityId, 'Upavasati');

  const [shakhes, upavasatis] = await Promise.all([loadScopedShakhes(), upavasatisPromise]);
  const shakhesByUpavasati = new Map();
  const placeByUpavasati = new Map();
  for (const shakhe of shakhes) {
    ...
    if (!shakhesByUpavasati.has(id)) {
      ...
      placeByUpavasati.set(id, placedFromShakhe(shakhe));
    }
  ...
  if (kind === 'with-shakhe') {
    ...
    for (const shakhe of shakhes) {
      ...
      seen.add(id);
      sourceUpavasatis.push({
        id,
        name: hierarchy.displayEntityName((shakhe.upavasati && shakhe.upavasati.name) || id),
      });
    }
  } else {
    sourceUpavasatis = upavasatis || [];
  }

  const missingPlaceIds = sourceUpavasatis
    .map((u) => u.id)
    .filter((id) => !placeByUpavasati.has(id));
  if (missingPlaceIds.length) {
    const ancestorsMap = await hierarchy.ancestorsByStharaForIds(missingPlaceIds);
    for (const id of missingPlaceIds) {
      placeByUpavasati.set(id, placedFromAncestors(ancestorsMap.get(id) || {}));
    }
  }

  const scopeId = String(entityId);
  const rows = sourceUpavasatis
    .map((upavasati) => {
      ...
      const placed = placeByUpavasati.get(upavasati.id) || {};
      // Drop cross-linked hierarchy rows that are not under this scope entity.
      const scopeRef =
        scopeSthara === 'Prant'
          ? null
          : scopeSthara === 'Vibhag'
            ...
            : scopeSthara === 'Bhag'
              ? placed.bhag
              : placed.nagar;
      if (scopeRef && String(scopeRef.id) !== scopeId) return null;
      return {
        id: upavasati.id,
        name: upavasati.name,
