...
async function reportForNagara(nagarId, fromDate, toDate, excludeSunday, vasatiId) {
  ...
  const dailyByGroup = await fixedWeekDailyCounts(
  ...
  const rows = rowTargets.map(({ vasati, upavasati, shakhes: vasatiShakhes }) => {
    ...
    return {
      ...
      dailyShakheCounts: dailyByGroup.get((upavasati || vasati).id) || {},
      averages: metrics.averages,
      ottuSamparka: metrics.ottuSamparka,
      _sums: metrics._sums,
    };
...
async function fixedWeekDailyCounts(shakhes, reportEntries, range, groupKey) {
  ...
  return new Map([...groups].map(([id, group]) => [id, dailyShakheCounts(group, byShakhe, DAILY_REPORT_DATES)]));
}
...
async function reportForParentLevel(level, entityId, fromDate, toDate, excludeSunday) {
  ...
  const dailyByGroup = await fixedWeekDailyCounts(rawShakhes, rawEntries, range, childKey);
  const rows = children.map((child) => {
    ...
    return {
      ...
      dailyShakheCounts: dailyByGroup.get(child.id) || {},
      averages,
      ottuSamparka,
    };
  });
