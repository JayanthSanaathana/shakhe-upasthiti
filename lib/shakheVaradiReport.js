...
async function descendantsGroupedByRoots(rootIds, targetSthara) {
  const groups = new Map(rootIds.map((id) => [id, []]));
  if (!rootIds.length) return groups;
  const target = await Sthara.findOne({ name: targetSthara }).select('_id').lean();
  if (!target) return groups;
  const targetId = target._id.toString();
  let frontier = rootIds.map((id) => ({ id, owner: id }));
  while (frontier.length) {
    const edges = await ParentEntity.find({ parentEntity: { $in: frontier.map((item) => item.id) } })
      .select('parentEntity currentEntity')
      .lean();
    if (!edges.length) break;
    const childIds = edges.map((edge) => edge.currentEntity);
    const children = await Entity.find({ _id: { $in: childIds } }).select('name sthara').lean();
    const childById = new Map(children.map((item) => [item._id.toString(), item]));
    const ownerById = new Map(frontier.map((item) => [item.id, item.owner]));
    const next = [];
    for (const edge of edges) {
      const child = childById.get(edge.currentEntity.toString());
      if (!child) continue;
      const owner = ownerById.get(edge.parentEntity.toString());
      if (!owner) continue;
      const childId = child._id.toString();
      if (child.sthara.toString() === targetId) groups.get(owner).push({ id: childId, name: displayEntityName(child.name) });
      else next.push({ id: childId, owner });
    }
    frontier = next;
  }
  return groups;
...
async function reportForNagara(nagarId, fromDate, toDate, excludeSunday, vasatiId) {
  ...
  const rows = rowTargets.map(({ vasati, upavasati, shakhes: vasatiShakhes }) => {
    ...
    return {
      ...
      upavasatiWithoutShakheCount: Math.max(0, metrics.upavasatiCount - [...withShakhe].filter((id) => gramIds.has(id)).length),
...
async function listUpavasatisForScope({ level, entityId, filter }) {
  ...
  const upavasatis = await hierarchy.descendantsOfSthara(entityId, 'Upavasati');
  const parentField = { prant: 'vibhag', vibhag: 'bhag', bhag: 'nagar', nagara: 'nagar' }[level];
  const shakhes = upavasatis.length
    ? await Shakhe.find({ [`${parentField}.entity`]: entityId, 'upavasati.entity': { $in: upavasatis.map((u) => u.id) } })
      .select('_id name upavasati')
      .lean()
    : [];
  ...
  for (const shakhe of shakhes) {
    ...
    if (!id) continue;
    if (!shakhesByUpavasati.has(id)) shakhesByUpavasati.set(id, []);
    shakhesByUpavasati.get(id).push({ id: shakhe._id.toString(), name: shakhe.name });
  }

  const parentByChild = new Map();
  const entityById = new Map();
  const stharaIds = new Set();
  let frontier = upavasatis.map((upavasati) => upavasati.id);
  const seen = new Set(frontier);
  while (frontier.length) {
    const edges = await ParentEntity.find({ currentEntity: { $in: frontier } })
      .select('currentEntity parentEntity')
      .lean();
    if (!edges.length) break;
    const parentIds = [...new Set(edges.map((edge) => edge.parentEntity.toString()))];
    const parents = await Entity.find({ _id: { $in: parentIds } }).select('name sthara').lean();
    for (const edge of edges) parentByChild.set(edge.currentEntity.toString(), edge.parentEntity.toString());
    for (const parent of parents) {
      entityById.set(parent._id.toString(), parent);
      stharaIds.add(parent.sthara.toString());
    }
    frontier = parentIds.filter((id) => !seen.has(id));
    for (const id of frontier) seen.add(id);
  }

  const stharas = stharaIds.size
    ? await Sthara.find({ _id: { $in: [...stharaIds] } }).select('name').lean()
    : [];
  const stharaNameById = new Map(stharas.map((sthara) => [sthara._id.toString(), sthara.name]));

  const rows = upavasatis.map((upavasati) => {
    const ancestors = {};
    let currentId = upavasati.id;
    while (parentByChild.has(currentId)) {
      const parentId = parentByChild.get(currentId);
      const parent = entityById.get(parentId);
      if (!parent) break;
      const stharaName = stharaNameById.get(parent.sthara.toString());
      if (stharaName) ancestors[stharaName] = parent;
      currentId = parentId;
    }
    ...
    return {
      id: upavasati.id,
      name: upavasati.name,
      vibhag: ancestors.Vibhag ? { id: ancestors.Vibhag._id.toString(), name: hierarchy.displayEntityName(ancestors.Vibhag.name) } : null,
      bhag: ancestors.Bhag ? { id: ancestors.Bhag._id.toString(), name: hierarchy.displayEntityName(ancestors.Bhag.name) } : null,
      nagar: ancestors.Nagar ? { id: ancestors.Nagar._id.toString(), name: hierarchy.displayEntityName(ancestors.Nagar.name) } : null,
      vasati: ancestors.Vasati ? { id: ancestors.Vasati._id.toString(), name: hierarchy.displayEntityName(ancestors.Vasati.name) } : null,
  ...
  return { level, entity: { id: root._id.toString(), name: hierarchy.displayEntityName(root.name) }, filter: kind, upavasatis: rows };
}
...
async function reportProgramForNagara(nagarId, programKind, fromDate, toDate, excludeSunday, itemDayCount, vasatiId) {
  ...
  const rows = rowTargets.map(({ vasati, upavasati, shakhes: vasatiShakhes }) => {
    ...
    return {
      ...
      upavasatiWithShakheCount: [...upavasatiWithShakhe].filter((id) => rowGrams.has(id)).length,
      upavasatiWithoutShakheCount: Math.max(0, rowGrams.size - [...upavasatiWithShakhe].filter((id) => rowGrams.has(id)).length),
...
async function reportForParentLevel(level, entityId, fromDate, toDate, excludeSunday) {
  ...
  const upavasatisByChild = new Map(
    [...(await descendantsGroupedByRoots(childIds, 'Upavasati')).entries()]
      .map(([id, upavasatis]) => [id, new Set(upavasatis.map((upavasati) => upavasati.id))])
  );

  // For vibhag/prant rows: map child → nagara ids under that child.
  let nagarIdsByChild = null;
  if (level === 'vibhag' || level === 'prant') {
    nagarIdsByChild = new Map(
      [...(await descendantsGroupedByRoots(childIds, 'Nagar')).entries()]
        .map(([id, nagars]) => [id, nagars.map((nagar) => nagar.id)])
    );
  }
  ...
  const rows = children.map((child) => {
    ...
    return {
      ...
      upavasatiWithShakheCount: [...upavasatiWithShakhe].filter((id) => totalUpavasatis.has(id)).length,
      upavasatiWithoutShakheCount: Math.max(0, totalUpavasatis.size - [...upavasatiWithShakhe].filter((id) => totalUpavasatis.has(id)).length),
...
async function reportProgramForParentLevel(level, entityId, programKind, fromDate, toDate, excludeSunday, itemDayCount) {
  ...
  const rawShakhes = childIds.length
    ? await Shakhe.find(shakheQuery).select('_id vibhag bhag nagar upavasati reportHideIntervals').lean()
    : [];
  const shakheIdsAll = rawShakhes.map((s) => s._id);
  const rawEntries = shakheIdsAll.length
    ? await ShakheUpasthiti.find({
