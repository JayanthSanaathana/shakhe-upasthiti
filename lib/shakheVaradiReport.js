...
async function listUpavasatisForScope({ level, entityId, filter }) {
  ...
  const shakhesPromise = Shakhe.find({ [`${parentField}.entity`]: entityId })
    .select('_id name upavasati vasati nagar bhag vibhag')
    .lean();
  ...
  const [shakhes, upavasatis] = await Promise.all([shakhesPromise, upavasatisPromise]);
