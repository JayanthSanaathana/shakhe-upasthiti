...
async function loadGramsByVasati(nagarId) {
  const idx = await hierarchy.ensureAdjacencyIndex();
  const vasatis = await hierarchy.descendantsOfSthara(nagarId, 'Vasati');
  const vasatiIds = vasatis.map((v) => v.id);
  const upavasatiStharaId = idx.stharaIdByName.get('Upavasati');
  if (!upavasatiStharaId) throw new Error('Unknown sthara: Upavasati');

  const gramsByVasati = new Map(vasatiIds.map((id) => [id, new Map()]));
  for (const vasatiId of vasatiIds) {
    const childIds = idx.childrenByParent.get(vasatiId) || [];
    const map = gramsByVasati.get(vasatiId);
    for (const childId of childIds) {
      if (idx.stharaByEntity.get(childId) !== upavasatiStharaId) continue;
      map.set(childId, {
        id: childId,
        name: displayEntityName(idx.nameByEntity.get(childId) || ''),
      });
    }
  }
...
async function parentEntityRef(entityId) {
  try {
    const idx = await hierarchy.ensureAdjacencyIndex();
    const parentId = idx.parentByChild.get(entityId.toString());
    if (parentId) {
      return {
        id: parentId,
