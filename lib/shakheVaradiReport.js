...
const DAILY_REPORT_DATES = Array.from({ length: 7 }, (_, index) => `2026-09-${21 + index}`);
const {
  shakheVisibleInDays,
...
function upavasatiSplitCounts(totalUpavasatis, withShakheIds) {
  const withCount = countInSet(withShakheIds, totalUpavasatis);
  return {
    upavasatiCount: totalUpavasatis.size,
    upavasatiWithShakheCount: withCount,
    upavasatiWithoutShakheCount: Math.max(0, totalUpavasatis.size - withCount),
  };
}
...
async function loadGramsByVasati(nagarId) {
  ...
  if (!vasatiIds.length) return { vasatis, gramsByVasati };

  // Live ParentEntity: all Upavasati children linked under each Vasati.
  const edges = await ParentEntity.find({ parentEntity: { $in: vasatiIds } })
    .select('parentEntity currentEntity')
    .lean();
  if (!edges.length) return { vasatis, gramsByVasati };
  ...
  for (const edge of edges) {
    ...
    if (!child || child.sthara.toString() !== upavasatiStharaId) continue;
    const vasatiId = edge.parentEntity.toString();
    const map = gramsByVasati.get(vasatiId);
    if (!map) continue;
    const id = child._id.toString();
    map.set(id, hierarchy.serializeEntity(child));
  }
...
async function parentEntityRef(entityId) {
  // Prefer a Nagar parent when multiple parent links exist.
  const edges = await ParentEntity.find({ currentEntity: entityId })
    .select('parentEntity')
    .lean();
  if (!edges.length) return null;
  const parents = await Entity.find({ _id: { $in: edges.map((e) => e.parentEntity) } })
    .select('name sthara')
    .lean();
  if (!parents.length) return null;
  const nagarSthara = await Sthara.findOne({ name: 'Nagar' }).select('_id').lean();
  const nagarStharaId = nagarSthara ? nagarSthara._id.toString() : '';
  const preferred =
    (nagarStharaId && parents.find((p) => p.sthara.toString() === nagarStharaId)) || parents[0];
  return {
    id: preferred._id.toString(),
    name: displayEntityName(preferred.name),
  };
}
...
async function reportForNagara(nagarId, fromDate, toDate, excludeSunday, vasatiId) {
  ...
  const [{ vasatis: allVasatis, gramsByVasati }, rawShakhes, bhag] = await Promise.all([
  ...
  const vasatis = vasatiId ? allVasatis.filter((v) => v.id === vasatiId) : allVasatis;
  if (vasatiId && !vasatis.length) return { error: 'Vasati not found', status: 404 };
  ...
  const rowTargets = vasatiId
    ? vasatis.flatMap((vasati) => [...(gramsByVasati.get(vasati.id) || new Map()).values()].map((upavasati) => ({
      vasati,
      upavasati,
      shakhes: (shakhesByVasati.get(vasati.id) || []).filter(
        (shakhe) => shakhe.upavasati && shakhe.upavasati.entity && shakhe.upavasati.entity.toString() === upavasati.id
      ),
    })))
    : vasatis.map((vasati) => ({
      vasati,
      upavasati: null,
      shakhes: shakhesByVasati.get(vasati.id) || [],
    }));

  const saptahaByGroup = await fixedWeekDailyCounts(
    vasatiId ? rawShakhes.filter((s) => s.vasati && String(s.vasati.entity) === vasatiId) : rawShakhes,
    rawEntries, range, vasatiId ? 'upavasati' : 'vasati'
  );
  const rows = rowTargets.map(({ vasati, upavasati, shakhes: vasatiShakhes }) => {
    const groupId = (upavasati || vasati).id;
    const grams = gramsByVasati.get(vasati.id) || new Map();
    const gramIds = upavasati ? new Set([upavasati.id]) : new Set(grams.keys());
    ...
    const withCount = countInSet(withShakhe, gramIds);
    return {
      vasati,
      ...(upavasati ? { upavasati } : {}),
      upavasatiCount: metrics.upavasatiCount,
      upavasatiWithShakheCount: withCount,
      upavasatiWithoutShakheCount: Math.max(0, metrics.upavasatiCount - withCount),
      ...
      dailyShakheCounts: saptahaByGroup.daily.get(groupId) || {},
      saptahaRanCount: saptahaByGroup.weekRan.get(groupId) || 0,
      saptahaNadayadaCount: saptahaByGroup.weekNadayada.get(groupId) || 0,
      averages: metrics.averages,
      ottuSamparka: metrics.ottuSamparka,
      _sums: metrics._sums,
    };
  ...
  const totalSums = rows.reduce((acc, row) => addEntrySums(acc, row._sums || emptySums()), emptySums());
  const totalDays = rows.reduce((n, row) => n + (row.daysRan || 0), 0);
  const totals = {
    ...
    dailyShakheCounts: sumDailyShakheCounts(rows, DAILY_REPORT_DATES),
    saptahaRanCount: sumSaptahaField(rows, 'saptahaRanCount'),
    saptahaNadayadaCount: sumSaptahaField(rows, 'saptahaNadayadaCount'),
    averages: {
  ...
  return {
    level: 'nagara',
    scope: vasatiId ? 'vasati' : 'nagara',
    dailyDates: DAILY_REPORT_DATES,
    nagar: { id: nagar._id.toString(), name: nagar.name },
    vasati: vasatiId ? { id: vasatis[0].id, name: vasatis[0].name } : null,
    bhag,
    from: range.from,
    to: range.to,
...
async function listUpavasatisForScope({ level, entityId, filter }) {
  if (!isObjectId(entityId)) return { error: 'Invalid entity' };
  if (!['prant', 'vibhag', 'bhag', 'nagara'].includes(level)) return { error: 'Invalid level' };
  const kind = filter || 'all';
  if (!['all', 'with-shakhe', 'without-shakhe'].includes(kind)) return { error: 'Invalid filter' };

  const root = await Entity.findById(entityId);
  if (!root) return { error: 'Entity not found', status: 404 };
  const expected = { prant: 'Prant', vibhag: 'Vibhag', bhag: 'Bhag', nagara: 'Nagar' }[level];
  if ((await hierarchy.stharaName(root)) !== expected) return { error: `Not a ${expected}` };

  // Shakhe docs only have vibhag/bhag/nagar — map the login/report scope onto those fields.
  async function loadScopedShakhes() {
    const select = '_id name timing time shakheType stanaName upavasati vasati nagar bhag vibhag';
    if (level === 'prant') {
      const vibhags = await hierarchy.descendantsOfSthara(entityId, 'Vibhag');
      const vibhagIds = vibhags.map((v) => v.id);
      if (!vibhagIds.length) return [];
      return Shakhe.find({ 'vibhag.entity': { $in: vibhagIds } }).select(select).lean();
    }
    const parentField = { vibhag: 'vibhag', bhag: 'bhag', nagara: 'nagar' }[level];
    return Shakhe.find({ [`${parentField}.entity`]: entityId }).select(select).lean();
  }

  // All three filters use the same reference paths. Saved Shakhe labels/parents
  // are snapshots and must not determine where an Upavasati belongs today.
  const [shakhes, sourceUpavasatis] = await Promise.all([
    loadScopedShakhes(), hierarchy.upavasatiPathsUnder(entityId),
  ]);
  const shakhesByUpavasati = new Map();
  for (const shakhe of shakhes) {
    const id = shakhe.upavasati && shakhe.upavasati.entity ? shakhe.upavasati.entity.toString() : '';
    if (!id) continue;
    if (!shakhesByUpavasati.has(id)) {
      shakhesByUpavasati.set(id, []);
    }
    shakhesByUpavasati.get(id).push({
      id: shakhe._id.toString(),
      name: shakhe.name,
      timing: shakhe.timing || '',
      time: shakhe.time || '',
      shakheType: shakhe.shakheType || '',
      stanaName: shakhe.stanaName || '',
      nagar: placedName(shakhe.nagar),
    });
  }

  const rows = sourceUpavasatis
    .map((upavasati) => {
      const itemShakhes = shakhesByUpavasati.get(upavasati.id) || [];
      if (kind === 'with-shakhe' && !itemShakhes.length) return null;
      if (kind === 'without-shakhe' && itemShakhes.length) return null;
      const placed = upavasati.placed;
      return {
        id: upavasati.id,
        name: upavasati.name,
        vibhag: placed.vibhag || null,
        bhag: placed.bhag || null,
        nagar: placed.nagar || null,
        vasati: placed.vasati || null,
        shakheCount: itemShakhes.length,
        shakhes: itemShakhes,
        hasShakhe: itemShakhes.length > 0,
      };
    })
    .filter(Boolean);
  ...
  return {
    level,
    entity: { id: root._id.toString(), name: hierarchy.displayEntityName(root.name) },
    filter: kind,
    upavasatis: rows,
  };
}
...
/** Count running shakhes that selected each program item at least once, or on an exact number of days. */
function itemShakheCountsFromGroup(catalog, shakhes, entriesByShakhe, field, itemDayCount) {
  const counts = emptyItemCounts(catalog);
  const exactDays = Number.isInteger(Number(itemDayCount)) && Number(itemDayCount) > 0 ? Number(itemDayCount) : null;
  for (const shakhe of shakhes || []) {
    const list = entriesByShakhe.get(shakhe._id.toString()) || [];
    if (!list.length) continue;
    const daysByItem = emptyItemCounts(catalog);
    for (const entry of list) {
      const items = Array.isArray(entry[field]) ? entry[field] : [];
      for (const id of items) {
        if (Object.prototype.hasOwnProperty.call(daysByItem, id)) daysByItem[id] += 1;
      }
    }
    for (const item of catalog) {
      const days = daysByItem[item.id] || 0;
      if (exactDays ? days === exactDays : days > 0) counts[item.id] += 1;
    }
  }
  return counts;
}
...
/** One Shakhe counts once per date, including a recorded zero-attendance entry. */
function dailyShakheCounts(shakhes, entriesByShakhe, days) {
  const counts = Object.fromEntries(days.map((day) => [day, 0]));
  const seen = new Set();
  for (const shakhe of shakhes) {
    const id = String(shakhe._id);
    if (seen.has(id)) continue;
    seen.add(id);
    const ran = new Set((entriesByShakhe.get(id) || []).map((entry) => entry.date));
    for (const day of ran) {
...
/** Unique shakhes that ran ≥1 day in the week, and those that ran 0 days. */
function saptahaWeekSummary(shakhes, entriesByShakhe, days) {
  let ran = 0;
  let total = 0;
  const seen = new Set();
  for (const shakhe of shakhes || []) {
    ...
    seen.add(id);
    total += 1;
    const dates = new Set((entriesByShakhe.get(id) || []).map((entry) => entry.date));
    if (days.some((day) => dates.has(day))) ran += 1;
  }
  return { ran, nadayada: Math.max(0, total - ran) };
}

function sumDailyShakheCounts(rows, days) {
  return Object.fromEntries(days.map((day) => [
    day, rows.reduce((sum, row) => sum + (row.dailyShakheCounts[day] || 0), 0),
  ]));
}

function sumSaptahaField(rows, field) {
  return (rows || []).reduce((sum, row) => sum + (Number(row[field]) || 0), 0);
}

/** Fixed campaign week, independent of the main range and Sunday exclusion. */
async function fixedWeekDailyCounts(shakhes, reportEntries, range, groupKey) {
  const from = DAILY_REPORT_DATES[0];
  const to = DAILY_REPORT_DATES[6];
  const rawEntries = range.from <= from && range.to >= to
    ? reportEntries.filter((entry) => entry.date >= from && entry.date <= to)
    : shakhes.length ? await ShakheUpasthiti.find({
      shakhe: { $in: shakhes.map((s) => s._id) }, date: { $gte: from, $lte: to },
    }).select('shakhe date').lean() : [];
  const entries = filterEntriesByShakheVisibility(rawEntries, intervalsByShakheId(shakhes));
  ...
  for (const shakhe of shakhes) {
    const id = shakhe[groupKey] && String(shakhe[groupKey].entity);
    if (!id) continue;
    if (!groups.has(id)) groups.set(id, []);
    groups.get(id).push(shakhe);
  }
  const daily = new Map();
  const weekRan = new Map();
  const weekNadayada = new Map();
  for (const [id, group] of groups) {
    daily.set(id, dailyShakheCounts(group, byShakhe, DAILY_REPORT_DATES));
    const summary = saptahaWeekSummary(group, byShakhe, DAILY_REPORT_DATES);
    weekRan.set(id, summary.ran);
    weekNadayada.set(id, summary.nadayada);
  }
  return { daily, weekRan, weekNadayada };
}

async function reportProgramForNagara(nagarId, programKind, fromDate, toDate, excludeSunday, itemDayCount, vasatiId) {
  if (!isObjectId(nagarId)) return { error: 'Invalid nagara' };
  ...
  const range = parseDateRange(fromDate, toDate, excludeSunday);
  if (range.error) return range;
  const requestedDays = itemDayCount == null || itemDayCount === '' ? null : Number(itemDayCount);
  if (requestedDays != null && (!Number.isInteger(requestedDays) || requestedDays < 1 || requestedDays > range.dayCount)) {
    return { error: 'Invalid item day count' };
  }
  ...
  const rows = rowTargets.map(({ vasati, upavasati, shakhes: vasatiShakhes }) => {
    const grams = gramsByVasati.get(vasati.id) || new Map();
    const rowGrams = upavasati ? new Map([[upavasati.id, upavasati]]) : grams;
    ...
    const itemCounts = itemShakheCountsFromGroup(catalog, vasatiShakhes, entriesByShakhe, field, requestedDays);
    const withCount = countInSet(upavasatiWithShakhe, rowGrams);
    return {
      vasati,
      ...(upavasati ? { upavasati } : {}),
      upavasatiCount: rowGrams.size,
      upavasatiWithShakheCount: withCount,
      upavasatiWithoutShakheCount: Math.max(0, rowGrams.size - withCount),
...
async function listShakhesForNagara({
  ...
  if (vasatiId && !isObjectId(vasatiId)) return { error: 'Invalid vasati' };
  if (upavasatiId && !isObjectId(upavasatiId)) return { error: 'Invalid upavasati' };
  ...
  const query = { 'nagar.entity': nagarId };
  if (vasatiId) query['vasati.entity'] = vasatiId;
  if (upavasatiId) query['upavasati.entity'] = upavasatiId;
...
async function reportForParentLevel(level, entityId, fromDate, toDate, excludeSunday) {
  ...
  const [rawShakhes, upavasatisByChild] = await Promise.all([
    childIds.length
      ? Shakhe.find(shakheQuery)
          .select('_id vibhag bhag nagar vasati upavasati reportHideIntervals')
          .lean()
      : Promise.resolve([]),
    hierarchy.descendantIdSetsByRoots(childIds, 'Upavasati'),
  ]);
  const shakheIdsAll = rawShakhes.map((s) => s._id);
  const rawEntries = shakheIdsAll.length
    ? await ShakheUpasthiti.find({
  ...
  // Derive nagar ids from loaded shakhes — avoids a second hierarchy walk.
  const nagarIdsByChild =
    level === 'vibhag' || level === 'prant'
      ? new Map(childIds.map((id) => [id, new Set()]))
      : null;
  for (const shakhe of shakhes) {
    ...
    if (nagarIdsByChild && cid && nagarIdsByChild.has(cid)) {
      const nid = shakhe.nagar && shakhe.nagar.entity ? shakhe.nagar.entity.toString() : '';
      if (nid) nagarIdsByChild.get(cid).add(nid);
    }
  }

  const saptahaByGroup = await fixedWeekDailyCounts(rawShakhes, rawEntries, range, childKey);
  const rows = children.map((child) => {
    const group = shakhesByChild.get(child.id) || [];
    const totalUpavasatis = upavasatisByChild.get(child.id) || new Set();
    let nadayuthiruva = 0;
    const upavasatiWithShakhe = new Set();
    let daysRan = 0;
    ...
    const split = upavasatiSplitCounts(totalUpavasatis, upavasatiWithShakhe);
    return {
      [childKey]: child,
      ...split,
      ...
      dailyShakheCounts: saptahaByGroup.daily.get(child.id) || {},
      saptahaRanCount: saptahaByGroup.weekRan.get(child.id) || 0,
      saptahaNadayadaCount: saptahaByGroup.weekNadayada.get(child.id) || 0,
      averages,
      ottuSamparka,
    };
  });
...
async function reportProgramForParentLevel(level, entityId, programKind, fromDate, toDate, excludeSunday, itemDayCount) {
  if (!isObjectId(entityId)) return { error: 'Invalid entity' };
  if (!['prant', 'vibhag', 'bhag'].includes(level)) return { error: 'Invalid level' };
  if (programKind !== 'boudhik' && programKind !== 'sharirik') {
    return { error: 'Invalid program kind' };
  ...
  const [rawShakhes, upavasatisByChild] = await Promise.all([
    childIds.length
      ? Shakhe.find(shakheQuery).select('_id vibhag bhag nagar upavasati reportHideIntervals').lean()
      : Promise.resolve([]),
    hierarchy.descendantIdSetsByRoots(childIds, 'Upavasati'),
  ]);
  const shakheIdsAll = rawShakhes.map((s) => s._id);
  ...
  const rows = children.map((child) => {
    ...
    const itemCounts = itemShakheCountsFromGroup(catalog, group, entriesByShakhe, field, requestedDays);
    const split = upavasatiSplitCounts(totalUpavasatis, upavasatiWithShakhe);
    return {
      [childKey]: child,
      ...split,
...
async function listProgramItemShakheSplit({
  ...
  if (upavasatiId) shakheQuery['upavasati.entity'] = upavasatiId;
...
async function listShakheStatusSplit({
  ...
  if (level === 'prant') {
    const vibhags = await hierarchy.descendantsOfSthara(entityId, 'Vibhag');
    shakheQuery['vibhag.entity'] = { $in: vibhags.map((v) => v.id) };
  }
  const rawShakhes = await Shakhe.find(shakheQuery)
    .select(
  ...
  for (const shakhe of shakhes) {
    ...
    const row = {
      ...
      upasthiti: range.from === range.to && list.length
        ? { date: range.from, ...compactUpasthiti(list[0]) }
        : null,
      averages: {
        taruna: avg(sums.taruna, daysRan),
        balaka: avg(sums.balaka, daysRan),
