...
async function listUpavasatisForScope({ level, entityId, filter }) {
  ...
  const rows = (await Promise.all(upavasatis.map(async (upavasati) => {
    const ancestors = await hierarchy.ancestorsBySthara(upavasati.id);
    const placed = (name) => ancestors[name]
      ? { id: ancestors[name]._id.toString(), name: hierarchy.displayEntityName(ancestors[name].name) }
      : null;
