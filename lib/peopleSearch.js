const { getEntityCollectionConnection } = require('./mongo');
...
async function searchPeople(phone) {
  ...
  if (!pattern) return [];
  const source = getEntityCollectionConnection('ssdatas');
  if (!source || source.readyState !== 1) return [];

  const sourcePeople = await source
    .collection('ssdatas')
  ...
  const sanghRows = await getEntityCollectionConnection('sanghdatas')
  ...
  const entities = nagarIds.length
    ? await getEntityCollectionConnection('entities')
        .collection('entities')
