...
function getEntityCollectionConnection(collection) {
  // ENTITY_REFERENCE_SOURCE=live → always read hierarchy/reference from live DB.
  // Named override (e.g. JALAHALLI) only affects sync preservation in entityCache,
  // not runtime reads — otherwise every Nagara stayed on stale local parententities.
  if (LOCAL_REFERENCE_COLLECTIONS.has(collection) && referenceDataSource() !== 'live') {
    return mongoose.connection;
  }
  // An explicit live source also takes precedence over the legacy full override.
  if (referenceDataSource() !== 'live' && LOCAL_HIERARCHY_COLLECTIONS.has(collection) && localHierarchyOverride()) {
    return mongoose.connection;
  }
...
async function connectMongo() {
  ...
  if (!liveUri || liveUri === process.env.MONGO_URI) {
    ...
    setImmediate(() => {
      require('./hierarchy')
        .warmAdjacencyIndex()
        .catch((err) => console.error('Hierarchy index warm failed:', err.message));
      require('./jalahalliNameOverlay')
        .warmOverlay()
        .catch((err) => console.error('Jalahalli name overlay warm failed:', err.message));
    });
  } else {
    ...
    // Do not block app.listen on a full cache rebuild. The entity collections
    // can be large, and Railway health checks need the HTTP listener quickly.
    // The refresh remains persistent and the scheduler continues daily syncs.
    syncEntityCacheIfNeeded(liveConn, mongoose.connection)
      .then((result) => {
        if (!result.skipped) {
          console.log(`Entity cache refreshed at ${result.syncedAt.toISOString()}`);
          try {
            require('./hierarchy').invalidateAdjacencyIndex();
        ...
        return Promise.all([
          require('./hierarchy').warmAdjacencyIndex(),
          require('./jalahalliNameOverlay').warmOverlay(),
        ]);
      })
      .catch((err) => console.error('Entity cache refresh failed:', err.message));
